...
import 'EventTurnoverFixedChart.dart';

class TurnoverFixedChart extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => _TurnoverFixedChart();
}

class _TurnoverFixedChart extends State<TurnoverFixedChart> {
  ...
  List<String> listNameTurnoverFixedView = ['Earnings', 'Business', 'Invest',];
  List<Color> listColor = [Color(0xff845bef), Color(0xff13d38e), Color(0xff0293ee),];

  Future<List<double>> list() async {
    List list = new List();
    List<double> listPercent = new List();

    list = await (ListUseEveryClass.listObject)[3].listMoneyStartEnd(_dateStart, _dateEnd);
  ...
  Future<List<List>> getData() async {
    List<List> listExpenditureFixed = await TurnoverFixed.turnoverFixed.listData(listNameTurnoverFixedView[touchedIndex],
        _dateStart, _dateEnd);
  ...
  Future<void> _awaitReturnValueFromCalendarStart(BuildContext context, EventTurnoverFixedChart eventTurnoverFixedChart) async {
    ...
    setState(() {
      ...
      if (this._dateStart == null) {
        eventTurnoverFixedChart.textDateStart = 'Date Start';
      }
      else {
        if (this._dateStart > this._dateEnd && this._dateEnd != 0) {
          ...
          eventTurnoverFixedChart.textDateStart = result['date'];
        }
      }
    });
    print(eventTurnoverFixedChart.textDateStart);
  }

  //Retrieve textDayEnd from Calendar
  Future<void> _awaitReturnValueFromCalendarEnd(BuildContext context, EventTurnoverFixedChart eventTurnoverFixedChart) async {
    final result = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => Calendar(),
    ...
    setState(() {
      ...
      if (this._dateEnd == null) {
        eventTurnoverFixedChart.textDateEnd = 'Date End';
        this._dateEnd = 0;
      }
      else {
        if (this._dateEnd >= this._dateStart) {
          eventTurnoverFixedChart.textDateEnd = result['date'];
        }
        else {
          this._dateEnd = 0;
          eventTurnoverFixedChart.textDateEnd = 'Date End';
    ...
    print(eventTurnoverFixedChart.textDateEnd);
  }

  Widget SetDateStatistic(BuildContext context, EventTurnoverFixedChart eventTurnoverFixedChart) {
    return Container(
        ...
        children: [
          Container(
            ...
            child: ListTile(
              ...
              title: Text(eventTurnoverFixedChart.textDateStart),
              onTap: () {
                _awaitReturnValueFromCalendarStart(context, eventTurnoverFixedChart);
              },
            ),
          ),
          Container(
            ...
            child: Container(
              ...
              child: ListTile(
                leading: Icon(Icons.date_range, color: Colors.deepPurpleAccent,),
                title: Text(eventTurnoverFixedChart.textDateEnd),
                onTap: () {
                  _awaitReturnValueFromCalendarEnd(context, eventTurnoverFixedChart);
                },
              ),
            ),
          ),
  ...
  Widget makeDropDownButtonSelectDate(BuildContext context, EventTurnoverFixedChart eventTurnoverFixedChart) {
      ...
      child: DropdownButton(
        ...
        onChanged: (String val) {
          ...
          setState(() {
            ...
            dayEndSelect = eventTurnoverFixedChart.dayEndofMonthSelect(monthSelect, yearSelect);
            String daySelectString = dayEndSelect.toString();

            eventTurnoverFixedChart.textDateStart = '1/' + monthSelectString + '/' + yearSelectString;
            eventTurnoverFixedChart.textDateEnd = daySelectString + '/' + monthSelectString + '/' + yearSelectString;
  ...
  Widget ThreeButtonDate(BuildContext context) {
    return ChangeNotifierProvider(
      create: (context) => EventTurnoverFixedChart(),
      child: Consumer<EventTurnoverFixedChart> (
  ...
  void deleteItem(List<String> listName, int index, int id) {
    ...
    (ListUseEveryClass.listObject[3]).deleteRow(id);
    setState(() {
      listMoney = list();
    });
  }
              ...
              if (!snapshot.hasData) {
                ...
                return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      String groupMoney = 'Turnover Fixed';
            ...
            child: FutureBuilder<List<double>>(
              ...
              builder: (BuildContext context, AsyncSnapshot<List<double>> snapshot) {
                if (!snapshot.hasData) {
                  ...
                  final items = snapshot.data;
                  //
                  int cnt = listNameTurnoverFixedView.length;
