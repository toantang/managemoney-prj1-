...
import 'package:maganemoney/listObjectUseEveryClass.dart';
import 'package:maganemoney/list_menu/Table_Calendar.dart';
import 'package:maganemoney/login/EventLogin.dart';
import 'package:provider/provider.dart';

import 'EventExpenditureFixedChart.dart';

class ExpenditureFixedChart extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => _ExpenditureFixedChart();
}

class _ExpenditureFixedChart extends State<ExpenditureFixedChart> {
  ...
  List<String> listNameExpenditureFixed = ['Electrics', 'Water', 'Internet', 'Interest', ];
  List<Color> listColor = [Color(0xff845bef), Colors.cyan[400], Color(0xff0293ee), Colors.lime[300]];

  Future<List<double>> list() async {
    List list = new List();
    List<double> listPercent = new List();
    list = await (ListUseEveryClass.listObject)[1].listMoneyStartEnd(_dateStart, _dateEnd);
  ...
  Future<void> _awaitReturnValueFromCalendarStart(BuildContext context, EventExpenditureFixedChart eventExpenditureFixedChart) async {
    ...
    setState(() {
      ...
      if (this._dateStart == null) {
        eventExpenditureFixedChart.textDateStart = 'Date Start';
      }
      else {
        if (this._dateStart > this._dateEnd && this._dateEnd != 0) {
          ...
          eventExpenditureFixedChart.textDateStart = result['date'];
        }
      }
    });
    print(eventExpenditureFixedChart.textDateStart);
  }

  //Retrieve textDayEnd from Calendar
  Future<void> _awaitReturnValueFromCalendarEnd(BuildContext context, EventExpenditureFixedChart eventExpenditureFixedChart) async {
    final result = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => Calendar(),
    ...
    setState(() {
      ...
      if (this._dateEnd == null) {
        eventExpenditureFixedChart.textDateEnd = 'Date End';
        this._dateEnd = 0;
      }
      else {
        if (this._dateEnd >= this._dateStart) {
          eventExpenditureFixedChart.textDateEnd = result['date'];
        }
        else {
          this._dateEnd = 0;
          eventExpenditureFixedChart.textDateEnd = 'Date End';
    ...
    print(eventExpenditureFixedChart.textDateEnd);
  }

  Widget SetDateStatistic(BuildContext context, EventExpenditureFixedChart eventExpenditureFixedChart) {
    return Container(
        ...
        children: [
          Container(
            ...
            child: ListTile(
              ...
              title: Text(eventExpenditureFixedChart.textDateStart),
              onTap: () {
                _awaitReturnValueFromCalendarStart(context, eventExpenditureFixedChart);
              },
            ),
          ),
          Container(
            ...
            child: Container(
              ...
              child: ListTile(
                leading: Icon(Icons.date_range, color: Colors.deepPurpleAccent,),
                title: Text(eventExpenditureFixedChart.textDateEnd),
                onTap: () {
                  _awaitReturnValueFromCalendarEnd(context, eventExpenditureFixedChart);
                },
              ),
            ),
          ),
  ...
  Widget makeDropDownButtonSelectDate(BuildContext context, EventExpenditureFixedChart eventExpenditureFixedChart) {
      ...
      child: DropdownButton(
        ...
        onChanged: (String val) {
          ...
          setState(() {
            ...
            dayEndSelect = eventExpenditureFixedChart.dayEndofMonthSelect(monthSelect, yearSelect);
            String daySelectString = dayEndSelect.toString();

            eventExpenditureFixedChart.textDateStart = '1/' + monthSelectString + '/' + yearSelectString;
            eventExpenditureFixedChart.textDateEnd = daySelectString + '/' + monthSelectString + '/' + yearSelectString;
  ...
  Widget ThreeButtonDate(BuildContext context) {
    return ChangeNotifierProvider(
      create: (context) => EventExpenditureFixedChart(),
      child: Consumer<EventExpenditureFixedChart> (
  ...
  Future<List<List>> getData() async {
    List<List> listExpenditureFixed = await ExpenditureFixed.expenditureFixed.listData(listNameExpenditureFixed[touchedIndex],
        _dateStart, _dateEnd);
  ...
  void deleteItem(List<String> listName, int index, int id) {
    ...
    (ListUseEveryClass.listObject[1]).deleteRow(id);
    setState(() {
      listMoney = list();
    });
  }
  ...
  Widget ShowItem(BuildContext context, List<String> listName, int index, String groupMoney, String nameDeal, String groupName,
      String money, printDateDeal, int id) {
    return Card(
              ...
              if (!snapshot.hasData) {
                ...
                return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      String groupMoney = 'Expenditure Fixed';
                      ...
                      return Dismissible(
                        ...
                        onDismissed: (direction) async {
                          ...
                          setState(() {
                            deleteItem(listName, index, index);
                          });
            ...
            child: FutureBuilder<List<double>>(
              ...
              builder: (BuildContext context, AsyncSnapshot<List<double>> snapshot) {
                if (!snapshot.hasData) {
                  ...
                  // số phần tử có giá trị = 0 trong listMoney
                  int cnt = listNameExpenditureFixed.length;

                  if (checkListisNaN(items) == false) {
                    int size = items.length;
                    return AspectRatio(
                      aspectRatio: 1,
                      child: makePieChart(context, items, cnt),
                    );
                  }
                  else {
                    return Center(
